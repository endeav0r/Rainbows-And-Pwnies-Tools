...
glibc_symbols = {}
for sec_name, sec in pairs(sublog["sections"]) do
    if sec["type"] == "symtab" then
        for name, symbol in pairs(sec["symbols"]) do
            if string.find(name, "@@GLIBC") then
                glibc_symbols[name] = symbol
            end
        end
    end
end
...
for name, symbol in pairs(glibc_symbols) do
    ...
    tmp[string.sub(name, 0, offset - 1)] = symbol
end
glibc_symbols = tmp
...
libc_symbols = {}
for name, sec in pairs(libc["sections"]) do
    if sec["type"] == "dynsym" then
        for libc_name, symbol in pairs(sec["symbols"]) do
            for glibc_name, glibc_symbol in pairs(glibc_symbols) do
                if libc_name == glibc_name then
                    libc_symbols[libc_name] = symbol
                end
            end
        end
    end
...
for k, rop in pairs(rop_table) do
    for name, libc_symbol in pairs(libc_symbols) do
        if not rop_added then
            if rop[1]["address"] < libc_symbol["address"] + ROP_OFFSET_SIZE and 
               rop[1]["address"] > libc_symbol["address"] - ROP_OFFSET_SIZE then
                ...
                valid_rops[name .. " " .. sign .. tostring(offset)] = rop
                break
            end
        end
    end
